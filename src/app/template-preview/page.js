...
import { Send, Info, MapPin, CheckCircle2 } from 'lucide-react';
import { Sheet, SheetContent, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";

export default function MasterTemplate() {
  // In production, this object will be fetched from Supabase via middleware
  const client = {
    business_name: "Sun City Staffing & Logistics",
    primary_color: "#ea580c", // Brand Hex Color (Orange)
    logo_url: "/assets/SCC_logo.png", // Reusing your existing logo for the preview
    industry: "recruiting",
  ...
  const [messages, setMessages] = useState([
    { role: 'ai', text: `Hi! I'm the digital recruiting assistant for ${client.business_name}. Are you looking for local El Paso routes or OTR?` }
  ]);
  ...
  const handleSendMessage = (e) => {
    ...
    // Simulate AI response for the preview
    setTimeout(() => {
      setMessages(prev => [...prev, { 
        role: 'ai', 
        text: "Got it! Do you currently hold a valid Class A CDL and have at least 1 year of experience?" 
      }]);
    }, 1000);
  };

  // Shared Informational Content (Used in both Desktop Left-Side and Mobile Drawer)
  const CompanyInfoBlock = () => (
    <div className="space-y-8 text-white">
      ...
      <section>
        <h3 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: client.primary_color }}>Why Drive For Us</h3>
  ...
  return (
    // h-dvh guarantees the UI never gets cut off by the mobile keyboard or Safari URL bar
    <main className="flex h-dvh w-full overflow-hidden bg-zinc-950 font-sans selection:bg-white/20">
      ...
      <section className="hidden lg:flex lg:w-1/2 flex-col relative border-r border-white/10 bg-zinc-900">
        {/* Subtle background glow using the client's primary color */}
        <div 
        ...
        <div className="flex-1 overflow-y-auto p-12 lg:p-16 z-10">
          <img src={client.logo_url} alt={client.business_name} className="h-16 w-auto mb-10" />
          <h1 className="text-5xl font-black text-white mb-10 leading-tight">
            Stop scrolling. <br/> Start driving.
          </h1>
          <CompanyInfoBlock />
        </div>
      </section>
      ...
      <section className="flex flex-col w-full lg:w-1/2 h-full relative bg-zinc-950">
        ...
        {/* Mobile-Only Sticky Header with Drawer Trigger */}
        <header className="lg:hidden flex items-center justify-between p-4 bg-zinc-950/80 backdrop-blur-md border-b border-white/10 shrink-0 z-20">
          <div className="flex items-center gap-3">
            <img src={client.logo_url} alt="Logo" className="h-8 w-auto" />
            <div>
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="bg-zinc-950 border-l border-white/10 w-[85%] sm:w-sm overflow-y-auto p-6">
              <SheetHeader className="text-left mt-4 mb-8">
                <img src={client.logo_url} alt={client.business_name} className="h-12 w-auto mb-4" />
